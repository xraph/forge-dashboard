...
- **In-app links use `PluginLink`, never a plain `<a>`.** The shell is a react-router single-page app, so a plain anchor is a full document load: capabilities refetched, every plugin remounted, the query store discarded. `PluginLink` falls back to a real anchor outside a host, so `getByRole("link")` and href assertions read the same in a test. `useNavigateTo()` covers the one case a link cannot, a page whose subject has just been deleted. Everything else stays a link so middle-click opens a tab.
...
- **A route and a slot contribution get their params differently, and confusing them is invisible until production.** `PluginHost` hands a ROUTE a single `params` prop. `PluginSlot` SPREADS a contribution's params onto it, so `<PluginSlot name="org.detail.tabs" params={{ orgId }} />` renders `<Contribution orgId="o1" />`, never `<Contribution params={...} />`. A contribution written to read `props.params.orgId` and tested through `renderSubPage` passes its test and renders nothing on a real page. Use `renderSubPage` for a route and `renderContribution` for a contribution: the second goes through a real `SubPluginProvider` and a real `PluginSlot`, so the tree under test is the production one.
